...
public class InformationSchemaScanner {
  ...
  private void listTables(Ddl.Builder builder) {
    ...
    switch (dialect) {
      case GOOGLE_STANDARD_SQL:
        ...
        break;
      case POSTGRESQL:
        query = Statement.of("SELECT t.table_name, t.parent_table_name, t.on_delete_action FROM"
  ...
  Statement listColumnsSQL() {
    switch (dialect) {
      case GOOGLE_STANDARD_SQL:
        return Statement.of("SELECT c.table_name, c.column_name,"
    ...
    while (resultSet.next()) {
      ...
      tableIndexes.put(
          ...
          Index.builder(dialect)
              ...
              .interleaveIn(parent).filter(filter));
    }
  }
    ...
    while (resultSet.next()) {
      ...
      ForeignKey.Builder foreignKey =
          tableForeignKeys.computeIfAbsent(
              name,
              k -> ForeignKey.builder(dialect).name(name).table(table).referencedTable(referencedTable));

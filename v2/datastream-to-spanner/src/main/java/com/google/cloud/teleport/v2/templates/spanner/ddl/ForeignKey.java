...
public abstract class ForeignKey implements Serializable {
  ...
  private void prettyPrint(Appendable appendable) throws IOException {
    ...
    String columnsString =
        columns().stream().map(c -> identifierQuote + c + identifierQuote).collect(Collectors.joining(", "));
    String referencedColumnsString =
        referencedColumns().stream().map(c -> identifierQuote + c + identifierQuote).collect(Collectors.joining(", "));
    appendable

...
2. Recursively materialises any child rows the schema declares
   (`Order` rows alongside each `User`), computing each child's primary
   key from the parent and pushing them into the same buffer.
3. Schedules N follow-up UPDATEs via a processing-time `Timer`, spaced
   `5 s` apart, where N is `updateQps / insertQps` for the table.
4. Optionally schedules a final DELETE after the last UPDATE.
5. Buffers each row by `(table, shard, op)`. When a buffer hits
   `--batchSize` it flushes — INSERTs in topological order, DELETEs in
   reverse-topological order, UPDATEs unordered.
6. On any sink-write failure, the failed batch is converted to one
   JSON `FailureRecord` per row and emitted to the dead-letter output
   stream. The bundle does **not** crash.

When the row's processing-time timer fires, the DoFn builds a fresh UPDATE row (PK preserved, all other columns regenerated) or a DELETE row (PK only, others nullable null) and buffers it.
...
* **Dynamic UPDATE interval compression**: when an ancestor row has a scheduled DELETE, the generator does not drop or cap the number of UPDATEs produced for that row's children. Instead, it dynamically compresses the timer interval between consecutive UPDATEs to guarantee that all scheduled UPDATEs complete strictly before the ancestor's DELETE.

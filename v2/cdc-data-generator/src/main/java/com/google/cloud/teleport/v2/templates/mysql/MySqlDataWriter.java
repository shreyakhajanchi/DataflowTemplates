...
public class MySqlDataWriter implements DataWriter {
  ...
  synchronized void ensureInitialized(int maxShardConnections) {
    ...
    if (!connectionHelper.isConnectionPoolInitialized()) {
      ...
      try {
        // Try 6-arg constructor
        java.lang.reflect.Constructor<ConnectionHelperRequest> constr6 =
        ...
        try {
          ...
          throw new RuntimeException("Failed to instantiate ConnectionHelperRequest", e2);
        }
      } catch (Exception e) {
        throw new RuntimeException("Failed to instantiate ConnectionHelperRequest", e);
      }

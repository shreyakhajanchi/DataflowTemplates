...
public class RowAssemblerTest {
  ...
  public void pkValuesOf_extractsInDeclaredOrder() {
    ...
    assertThat(pk.keySet()).containsExactly("a", "b").inOrder();
    assertThat(pk).containsEntry("a", 1L);
  ...
  public void pkValuesOf_emptyWhenNoPkOnTable() {
    ...
    assertThat(RowAssembler.pkValuesOf(row, table)).isEmpty();
  }

  // ===========================================================================
  // stateKeyOf + canonicalizeValue
  // ===========================================================================

  @Test
  public void stateKeyOf_singleStringPk() {
    LinkedHashMap<String, Object> pk = new LinkedHashMap<>();
    pk.put("id", "user-1");
    assertThat(RowAssembler.stateKeyOf("Users", pk)).isEqualTo("Users:user-1");
  }

  @Test
  public void stateKeyOf_compositePk_joinsWithHash() {
    LinkedHashMap<String, Object> pk = new LinkedHashMap<>();
    pk.put("a", 1L);
    pk.put("b", "x");
    assertThat(RowAssembler.stateKeyOf("T", pk)).isEqualTo("T:1#x");
  }
  ...
  @Test
  public void stateKeyOf_byteArrayPk_isStableAcrossInstances() {
    // toString() on byte[] returns "[B@<hashcode>" and is NOT stable across JVMs / GCs.
    // canonicalizeValue must produce the same key for two distinct byte[] instances with the
    // same contents.
    LinkedHashMap<String, Object> pk1 = new LinkedHashMap<>();
    pk1.put("id", new byte[] {1, 2, 3});
    LinkedHashMap<String, Object> pk2 = new LinkedHashMap<>();
    pk2.put("id", new byte[] {1, 2, 3});
    assertThat(RowAssembler.stateKeyOf("T", pk1)).isEqualTo(RowAssembler.stateKeyOf("T", pk2));
  }
  ...
  @Test
  public void stateKeyOf_nullValue() {
    LinkedHashMap<String, Object> pk = new LinkedHashMap<>();
    pk.put("a", null);
    assertThat(RowAssembler.stateKeyOf("T", pk)).isEqualTo("T:null");
  }
  ...
  @Test
  public void generateUpdateRow_fkColumnsPreservedFromOriginalWhenAvailable() {
    DataGeneratorForeignKey fk =
        DataGeneratorForeignKey.builder()
            ...
            .build();
    DataGeneratorTable table =
        baseTable("Child", ImmutableList.of("id"))
            .columns(ImmutableList.of(intColumn("id"), intColumn("parent_id")))
            .foreignKeys(ImmutableList.of(fk))
            .build();
    Schema origSchema = Schema.builder().addInt64Field("id").addInt64Field("parent_id").build();
    Row original = Row.withSchema(origSchema).addValues(7L, 42L).build();

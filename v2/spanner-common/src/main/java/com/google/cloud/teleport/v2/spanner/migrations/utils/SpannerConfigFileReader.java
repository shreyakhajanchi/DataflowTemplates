...
import com.google.cloud.teleport.v2.spanner.migrations.spanner.SpannerConnectionConfig;
...
/**
 * Class to read the Cloud Spanner configuration file in GCS and convert it into a
 * SpannerConnectionConfig object.
 */
public class SpannerConfigFileReader {
  ...
  public SpannerConnectionConfig getSpannerConfig(String spannerConfigFilePath) {
    try (InputStream stream =
        Channels.newInputStream(
            FileSystems.open(FileSystems.matchNewResource(spannerConfigFilePath, false)))) {

      String result = IOUtils.toString(stream, StandardCharsets.UTF_8);
      SpannerConnectionConfig config =
          new GsonBuilder()
              .setFieldNamingPolicy(FieldNamingPolicy.IDENTITY)
              .create()
              .fromJson(result, SpannerConnectionConfig.class);
